...
func (x *SockType) save(m state.Map) {
	m.SaveValue("", (int)(*x))
}

func (x *SockType) load(m state.Map) {
	m.LoadValue("", new(int), func(y interface{}) { *x = (SockType)(y.(int)) })
}
...
func init() {
	...
	state.Register("unix.SockType", (*SockType)(nil), state.Fns{Save: (*SockType).save, Load: (*SockType).load})
