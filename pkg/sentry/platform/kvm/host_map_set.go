...
const (
	...
	hostMapmaxDegree = 2 * hostMapminDegree
)
...
func (s *hostMapSet) IsEmpty() bool {
...
// more efficient.
func (s *hostMapSet) IsEmptyRange(r __generics_imported0.AddrRange) bool {
	switch {
	case r.Length() < 0:
...
func (s *hostMapSet) Span() __generics_imported0.Addr {
...
func (s *hostMapSet) SpanRange(r __generics_imported0.AddrRange) __generics_imported0.Addr {
	switch {
	case r.Length() < 0:
		panic(fmt.Sprintf("invalid range %v", r))
	case r.Length() == 0:
...
func (s *hostMapSet) FirstSegment() hostMapIterator {
	if s.root.nrSegments == 0 {
		return hostMapIterator{}
	}
...
func (s *hostMapSet) LastSegment() hostMapIterator {
	if s.root.nrSegments == 0 {
		return hostMapIterator{}
	}
...
func (s *hostMapSet) FirstGap() hostMapGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[0]
	}
	return hostMapGapIterator{n, 0}
}

// LastGap returns the last gap in the set.
func (s *hostMapSet) LastGap() hostMapGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return hostMapGapIterator{n, n.nrSegments}
}
...
func (s *hostMapSet) Find(key __generics_imported0.Addr) (hostMapIterator, hostMapGapIterator) {
	n := &s.root
	for {
		...
		for lower < upper {
			...
			if r := n.keys[i]; key < r.End {
				if key >= r.Start {
					return hostMapIterator{n, i}, hostMapGapIterator{}
				}
				upper = i
			} else {
				lower = i + 1
		...
		i := lower
		if !n.hasChildren {
			return hostMapIterator{}, hostMapGapIterator{n, i}
		}
		n = n.children[i]
	}
}
...
func (s *hostMapSet) FindSegment(key __generics_imported0.Addr) hostMapIterator {
	seg, _ := s.Find(key)
	return seg
}
...
func (s *hostMapSet) LowerBoundSegment(min __generics_imported0.Addr) hostMapIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
		return seg
	}
...
func (s *hostMapSet) UpperBoundSegment(max __generics_imported0.Addr) hostMapIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
		return seg
	}
...
// terminal iterator.
func (s *hostMapSet) FindGap(key __generics_imported0.Addr) hostMapGapIterator {
	_, gap := s.Find(key)
	return gap
}
...
// equal to min.
func (s *hostMapSet) LowerBoundGap(min __generics_imported0.Addr) hostMapGapIterator {
	seg, gap := s.Find(min)
	if gap.Ok() {
		return gap
	}
...
// equal to max.
func (s *hostMapSet) UpperBoundGap(max __generics_imported0.Addr) hostMapGapIterator {
	seg, gap := s.Find(max)
	if gap.Ok() {
		return gap
	}
...
func (s *hostMapSet) Add(r __generics_imported0.AddrRange, val uintptr) bool {
	if r.Length() <= 0 {
...
// invalidated.
func (s *hostMapSet) AddWithoutMerging(r __generics_imported0.AddrRange, val uintptr) bool {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	gap := s.FindGap(r.Start)
...
func (s *hostMapSet) Insert(gap hostMapGapIterator, r __generics_imported0.AddrRange, val uintptr) hostMapIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (hostMapSetFunctions{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			prev.SetEndUnchecked(r.End)
			prev.SetValue(mval)
			if next.Ok() && next.Start() == r.End {
				val = mval
				if mval, ok := (hostMapSetFunctions{}).Merge(prev.Range(), val, next.Range(), next.Value()); ok {
	...
	if next.Ok() && next.Start() == r.End {
		if mval, ok := (hostMapSetFunctions{}).Merge(r, val, next.Range(), next.Value()); ok {
...
func (s *hostMapSet) InsertWithoutMerging(gap hostMapGapIterator, r __generics_imported0.AddrRange, val uintptr) hostMapIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *hostMapSet) InsertWithoutMergingUnchecked(gap hostMapGapIterator, r __generics_imported0.AddrRange, val uintptr) hostMapIterator {
	...
	gap.node.nrSegments++
	return hostMapIterator{gap.node, gap.index}
}

// Remove removes the given segment and returns an iterator to the vacated gap.
// All existing iterators (including seg, but not including the returned
// iterator) are invalidated.
func (s *hostMapSet) Remove(seg hostMapIterator) hostMapGapIterator {
	...
	hostMapSetFunctions{}.ClearValue(&seg.node.values[seg.node.nrSegments-1])
	seg.node.nrSegments--
	return seg.node.rebalanceAfterRemove(hostMapGapIterator{seg.node, seg.index})
}

// RemoveAll removes all segments from the set. All existing iterators are
// invalidated.
func (s *hostMapSet) RemoveAll() {
	s.root = hostMapnode{}
}

// RemoveRange removes all segments in the given range. An iterator to the
// newly formed gap is returned, and all existing iterators are invalidated.
func (s *hostMapSet) RemoveRange(r __generics_imported0.AddrRange) hostMapGapIterator {
...
func (s *hostMapSet) Merge(first, second hostMapIterator) hostMapIterator {
...
func (s *hostMapSet) MergeUnchecked(first, second hostMapIterator) hostMapIterator {
	if first.End() == second.Start() {
		if mval, ok := (hostMapSetFunctions{}).Merge(first.Range(), first.Value(), second.Range(), second.Value()); ok {
...
func (s *hostMapSet) MergeRange(r __generics_imported0.AddrRange) {
...
func (s *hostMapSet) MergeAdjacent(r __generics_imported0.AddrRange) {
...
// Split panics.
func (s *hostMapSet) Split(seg hostMapIterator, split __generics_imported0.Addr) (hostMapIterator, hostMapIterator) {
...
func (s *hostMapSet) SplitUnchecked(seg hostMapIterator, split __generics_imported0.Addr) (hostMapIterator, hostMapIterator) {
	val1, val2 := (hostMapSetFunctions{}).Split(seg.Range(), seg.Value(), split)
	end2 := seg.End()
...
func (s *hostMapSet) SplitAt(split __generics_imported0.Addr) bool {
...
// including the returned iterators) are invalidated.
func (s *hostMapSet) Isolate(seg hostMapIterator, r __generics_imported0.AddrRange) hostMapIterator {
...
func (s *hostMapSet) ApplyContiguous(r __generics_imported0.AddrRange, fn func(seg hostMapIterator)) hostMapGapIterator {
	seg, gap := s.Find(r.Start)
	if !seg.Ok() {
		return gap
	}
...
type hostMapnode struct {
	...
	keys     [hostMapmaxDegree - 1]__generics_imported0.AddrRange
	values   [hostMapmaxDegree - 1]uintptr
	children [hostMapmaxDegree]*hostMapnode
}

// firstSegment returns the first segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *hostMapnode) firstSegment() hostMapIterator {
	for n.hasChildren {
		n = n.children[0]
	}
	return hostMapIterator{n, 0}
}

// lastSegment returns the last segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *hostMapnode) lastSegment() hostMapIterator {
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return hostMapIterator{n, n.nrSegments - 1}
}

func (n *hostMapnode) prevSibling() *hostMapnode {
...
func (n *hostMapnode) nextSibling() *hostMapnode {
...
// represented by gap.
func (n *hostMapnode) rebalanceBeforeInsert(gap hostMapGapIterator) hostMapGapIterator {
	if n.parent != nil {
		gap = n.parent.rebalanceBeforeInsert(gap)
	}
	if n.nrSegments < hostMapmaxDegree-1 {
		return gap
	}
	if n.parent == nil {

		left := &hostMapnode{
			nrSegments:  hostMapminDegree - 1,
			parent:      n,
			parentIndex: 0,
		...
		copy(left.keys[:hostMapminDegree-1], n.keys[:hostMapminDegree-1])
		copy(left.values[:hostMapminDegree-1], n.values[:hostMapminDegree-1])
		copy(right.keys[:hostMapminDegree-1], n.keys[hostMapminDegree:])
		copy(right.values[:hostMapminDegree-1], n.values[hostMapminDegree:])
		n.keys[0], n.values[0] = n.keys[hostMapminDegree-1], n.values[hostMapminDegree-1]
		hostMapzeroValueSlice(n.values[1:])
		if n.hasChildren {
			copy(left.children[:hostMapminDegree], n.children[:hostMapminDegree])
			copy(right.children[:hostMapminDegree], n.children[hostMapminDegree:])
			hostMapzeroNodeSlice(n.children[2:])
			for i := 0; i < hostMapminDegree; i++ {
		...
		if gap.index < hostMapminDegree {
			return hostMapGapIterator{left, gap.index}
		}
		return hostMapGapIterator{right, gap.index - hostMapminDegree}
	}
	...
	n.parent.keys[n.parentIndex], n.parent.values[n.parentIndex] = n.keys[hostMapminDegree-1], n.values[hostMapminDegree-1]
	...
	n.parent.nrSegments++
	copy(sibling.keys[:hostMapminDegree-1], n.keys[hostMapminDegree:])
	copy(sibling.values[:hostMapminDegree-1], n.values[hostMapminDegree:])
	hostMapzeroValueSlice(n.values[hostMapminDegree-1:])
	if n.hasChildren {
		copy(sibling.children[:hostMapminDegree], n.children[hostMapminDegree:])
		hostMapzeroNodeSlice(n.children[hostMapminDegree:])
		for i := 0; i < hostMapminDegree; i++ {
			sibling.children[i].parent = sibling
			sibling.children[i].parentIndex = i
		}
	}
	n.nrSegments = hostMapminDegree - 1
	...
	return hostMapGapIterator{sibling, gap.index - hostMapminDegree}
}
...
// B-tree invariant.
func (n *hostMapnode) rebalanceAfterRemove(gap hostMapGapIterator) hostMapGapIterator {
	for {
		if n.nrSegments >= hostMapminDegree-1 {
			return gap
		}
		if n.parent == nil {
		...
		if sibling := n.prevSibling(); sibling != nil && sibling.nrSegments >= hostMapminDegree {
			...
			hostMapSetFunctions{}.ClearValue(&sibling.values[sibling.nrSegments-1])
			if n.hasChildren {
			...
			if gap.node == n {
				return hostMapGapIterator{n, gap.index + 1}
			}
			return gap
		}
		if sibling := n.nextSibling(); sibling != nil && sibling.nrSegments >= hostMapminDegree {
			...
			if gap.node == sibling {
				...
				return hostMapGapIterator{sibling, gap.index - 1}
			}
			return gap
		}
		...
		if p.nrSegments == 1 {
			...
			if gap.node == left {
				return hostMapGapIterator{p, gap.index}
			}
			if gap.node == right {
				return hostMapGapIterator{p, gap.index + left.nrSegments + 1}
			}
			return gap
		}
		...
		if gap.node == right {
			gap = hostMapGapIterator{left, gap.index + left.nrSegments + 1}
		}
		...
		hostMapSetFunctions{}.ClearValue(&p.values[p.nrSegments-1])
...
func (seg hostMapIterator) Ok() bool {
	return seg.node != nil
}

// Range returns the iterated segment's range key.
func (seg hostMapIterator) Range() __generics_imported0.AddrRange {
...
func (seg hostMapIterator) Start() __generics_imported0.Addr {
	return seg.node.keys[seg.index].Start
}

// End is equivalent to Range().End, but should be preferred if only the end of
// the range is needed.
func (seg hostMapIterator) End() __generics_imported0.Addr {
...
func (seg hostMapIterator) SetRangeUnchecked(r __generics_imported0.AddrRange) {
...
// iterators.
func (seg hostMapIterator) SetRange(r __generics_imported0.AddrRange) {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (seg hostMapIterator) SetStartUnchecked(start __generics_imported0.Addr) {
...
// iterators.
func (seg hostMapIterator) SetStart(start __generics_imported0.Addr) {
	if start >= seg.End() {
...
func (seg hostMapIterator) SetEndUnchecked(end __generics_imported0.Addr) {
...
// iterators.
func (seg hostMapIterator) SetEnd(end __generics_imported0.Addr) {
	if end <= seg.Start() {
...
func (seg hostMapIterator) Value() uintptr {
...
// invalidate any iterators.
func (seg hostMapIterator) ValuePtr() *uintptr {
	return &seg.node.values[seg.index]
}

// SetValue mutates the iterated segment's value. This operation does not
// invalidate any iterators.
func (seg hostMapIterator) SetValue(val uintptr) {
...
func (seg hostMapIterator) PrevSegment() hostMapIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index].lastSegment()
	}
	if seg.index > 0 {
		return hostMapIterator{seg.node, seg.index - 1}
	}
	if seg.node.parent == nil {
		return hostMapIterator{}
	}
	return hostMapsegmentBeforePosition(seg.node.parent, seg.node.parentIndex)
}
...
func (seg hostMapIterator) NextSegment() hostMapIterator {
	if seg.node.hasChildren {
	...
	return hostMapsegmentAfterPosition(seg.node.parent, seg.node.parentIndex)
}

// PrevGap returns the gap immediately before the iterated segment.
func (seg hostMapIterator) PrevGap() hostMapGapIterator {
	if seg.node.hasChildren {

		return seg.node.children[seg.index].lastSegment().NextGap()
	}
	return hostMapGapIterator{seg.node, seg.index}
}

// NextGap returns the gap immediately after the iterated segment.
func (seg hostMapIterator) NextGap() hostMapGapIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index+1].firstSegment().PrevGap()
	}
	return hostMapGapIterator{seg.node, seg.index + 1}
}
...
// non-terminal.
func (seg hostMapIterator) PrevNonEmpty() (hostMapIterator, hostMapGapIterator) {
	gap := seg.PrevGap()
	if gap.Range().Length() != 0 {
		return hostMapIterator{}, gap
	}
	return gap.PrevSegment(), hostMapGapIterator{}
}
...
// non-terminal.
func (seg hostMapIterator) NextNonEmpty() (hostMapIterator, hostMapGapIterator) {
	gap := seg.NextGap()
	if gap.Range().Length() != 0 {
		return hostMapIterator{}, gap
	}
	return gap.NextSegment(), hostMapGapIterator{}
}
...
// Unless otherwise specified, any mutation of a set invalidates all existing
// iterators into the set.
type hostMapGapIterator struct {
...
// Ok returns true if the iterator is not terminal. All other methods are only
// valid for non-terminal iterators.
func (gap hostMapGapIterator) Ok() bool {
	return gap.node != nil
}

// Range returns the range spanned by the iterated gap.
func (gap hostMapGapIterator) Range() __generics_imported0.AddrRange {
	return __generics_imported0.AddrRange{gap.Start(), gap.End()}
}

// Start is equivalent to Range().Start, but should be preferred if only the
// start of the range is needed.
func (gap hostMapGapIterator) Start() __generics_imported0.Addr {
	if ps := gap.PrevSegment(); ps.Ok() {
		return ps.End()
	}
	return hostMapSetFunctions{}.MinKey()
}

// End is equivalent to Range().End, but should be preferred if only the end of
// the range is needed.
func (gap hostMapGapIterator) End() __generics_imported0.Addr {
	if ns := gap.NextSegment(); ns.Ok() {
		return ns.Start()
	}
	return hostMapSetFunctions{}.MaxKey()
}

// IsEmpty returns true if the iterated gap is empty (that is, the "gap" is
// between two adjacent segments.)
func (gap hostMapGapIterator) IsEmpty() bool {
...
func (gap hostMapGapIterator) PrevSegment() hostMapIterator {
	return hostMapsegmentBeforePosition(gap.node, gap.index)
}
...
func (gap hostMapGapIterator) NextSegment() hostMapIterator {
	return hostMapsegmentAfterPosition(gap.node, gap.index)
}

// PrevGap returns the iterated gap's predecessor. If no such gap exists,
// PrevGap returns a terminal iterator.
func (gap hostMapGapIterator) PrevGap() hostMapGapIterator {
	seg := gap.PrevSegment()
	if !seg.Ok() {
		return hostMapGapIterator{}
	}
...
// returns a terminal iterator.
func (gap hostMapGapIterator) NextGap() hostMapGapIterator {
	seg := gap.NextSegment()
	if !seg.Ok() {
		return hostMapGapIterator{}
	}
...
func hostMapsegmentBeforePosition(n *hostMapnode, i int) hostMapIterator {
	for i == 0 {
		if n.parent == nil {
			return hostMapIterator{}
		}
...
func hostMapsegmentAfterPosition(n *hostMapnode, i int) hostMapIterator {
	for i == n.nrSegments {
		if n.parent == nil {
			return hostMapIterator{}
		}
	...
	return hostMapIterator{n, i}
}

func hostMapzeroValueSlice(slice []uintptr) {

	for i := range slice {
		hostMapSetFunctions{}.ClearValue(&slice[i])
	}
}

func hostMapzeroNodeSlice(slice []*hostMapnode) {
	for i := range slice {
		slice[i] = nil
	}
}
...
func (s *hostMapSet) String() string {
	return s.root.String()
}

// String stringifes a node (and all of its children) for debugging.
func (n *hostMapnode) String() string {
	var buf bytes.Buffer
	n.writeDebugString(&buf, "")
	return buf.String()
}

func (n *hostMapnode) writeDebugString(buf *bytes.Buffer, prefix string) {
...
// key order.
func (s *hostMapSet) ExportSortedSlices() *hostMapSegmentDataSlices {
	var sds hostMapSegmentDataSlices
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
func (s *hostMapSet) ImportSortedSlices(sds *hostMapSegmentDataSlices) error {
	if !s.IsEmpty() {
...
func (s *hostMapSet) saveRoot() *hostMapSegmentDataSlices {
	return s.ExportSortedSlices()
}

func (s *hostMapSet) loadRoot(sds *hostMapSegmentDataSlices) {
