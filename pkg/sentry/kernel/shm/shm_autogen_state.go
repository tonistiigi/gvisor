...
func (x *AttachOpts) beforeSave() {}
func (x *AttachOpts) save(m state.Map) {
	x.beforeSave()
	m.Save("Execute", &x.Execute)
	m.Save("Readonly", &x.Readonly)
	m.Save("Remap", &x.Remap)
}

func (x *AttachOpts) afterLoad() {}
func (x *AttachOpts) load(m state.Map) {
	m.Load("Execute", &x.Execute)
	m.Load("Readonly", &x.Readonly)
	m.Load("Remap", &x.Remap)
}
...
func init() {
	...
	state.Register("shm.AttachOpts", (*AttachOpts)(nil), state.Fns{Save: (*AttachOpts).save, Load: (*AttachOpts).load})
}
